...
# This application provides a GUI for editing certain system settings that 
# aren't typically available by default in GNOME. The initial release will 
# handle listing all audio devices, provide the user the ability to rename
# them, and to hide them as desired. 

class CustomTitleBar(QWidget):
    def __init__(self, parent):
        super().__init__(parent)

        # Title Bar General Properties
        ...
        # Add all buttons to the title bar
        buttons = [
            self.min_button,
            self.normal_button,
            self.max_button,
    ...
    def window_state_changed(self, state):
        """
        Show or hide the maximize button depending on the window state, and
        Show or hide the normal button depending on the window state.
        """
    ...
    def mousePressEvent(self, event):
        """
        Capture the initial position of the mouse when the left mouse button
        is pressed.
        """
    ...
    def mouseMoveEvent(self, event):
        """
        Move the window when the mouse is moved while the left mouse button is
        pressed.
        """
    ...
    def mouseReleaseEvent(self, event):
        """
        Reset the initial position of the mouse when the left mouse button is
        released.
        """
        self.initial_pos = None
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        light_mode_toolbar = f"""
            background-color: {vlg}; 
            color: {vlg}; 
            font-size: 14px;
        """
        dark_mode_toolbar = f"""
            background-color: {vdg}; 
            color: {w}; 
            font-size: 14px;
        """
        highlighted_toolbar = f"""
            background-color: {h}; 
            color: {w}; 
            font-size: 14px;
        """
        light_mode_central = f"""
            background-color: {lg}; 
            color: {dg}; 
            font-size: 14px; 
            border-radius: 7px;
        ...
        screen_height: int = screen_geometry.height()
            # Retrieve screen geometry in order to specify initial window size.

        QApplication.setFont(QFontDatabase.systemFont(
            QFontDatabase.SystemFont.GeneralFont)
            )
            # Set the default font for the application by using what the base
        ...
        #  Set central widget's general properties.
        central_widget = QWidget()
        central_widget.setObjectName("Container")
        central_widget.setStyleSheet(light_mode_central)

        #  Set central widget's initial size.
        ...
        # Set the central widget's initial layout.
    ...
    def changeEvent(self, event):
        """
        This function is called whenever the window's state changes.
        """
...
def gnome_check () -> bool:
    """
    This function checks to see if the current desktop session is GNOME, and 
    returns a boolean along with the value of the associated environment 
    variable.
    """
    ...
    if current_desktop_session == 'gnome':
        ...
        msg_box_a.setText(f"""
                          Only GNOME Desktop Sessions are supported. Your 
                          current desktop session is: 
